...
// 기존행 클리어 -> 조회 -> 바인딩
Future<void> palletCommonViewPackingList(
...
Future<void> palletCommonViewTopList(
  ...
  //바인딩
  if (pallets == null) {
    showCustomSnackBarWarn(context, '해당 작업위치에 입력완료한 실적이 없습니다.');
  } else {
    gridStateManager.appendRows(
...
void deletePackItem(
  ...
  showAlertDialog(context, '정상처리 되었습니다.');
}

//최초로딩시 콤보박스 세팅
//데이터가 없는 경우 하단 알람 창에 메세지 전시
Future<List<ComboValueType>> palletCommonGetLocationComboValueList(BuildContext context) async {
