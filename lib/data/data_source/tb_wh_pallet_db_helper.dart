...
class TbWhPalletDbHelper {
  ...
  Result<bool> updateTbWhPalletState(TbWhPallet pallet) {
    ...
    print('updateTbWhPalletState : ${pallet.state}, ${pallet.palletSeq}, ${pallet.comps} , ${pallet.boxNo}, ${pallet.itemLot}');
  ...
  Future<bool> deleteTbWhPallet(TbWhPallet pallet) async {
    try {
      int nRet = await db.delete(
        'TB_WH_PALLET',
        where: 'comps = ? and palletSeq = ? and boxNo = ? and itemLot = ?',
        whereArgs: [
          pallet.comps,
          pallet.palletSeq,
          pallet.boxNo,
  ...
  Future<int> getTbWhPalletCountInDevice() async {
    int count = Sqflite.firstIntValue(
        await db.rawQuery('SELECT COUNT(*) FROM TB_WH_PALLET '))!;
    return count;
  }

  Future<Result<bool>> checkTbWhPalletDuplication(String sQrCode) async {
    int? count = Sqflite.firstIntValue(await db.rawQuery(
        'SELECT COUNT(*) FROM TB_WH_PALLET WHERE qrCode = ?', [sQrCode]));
